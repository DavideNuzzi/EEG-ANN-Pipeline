...
from models.custom_blocks import EEGConvBlock, EEGConvBlock_V2
...
class EEGMultiScaleClassifier_V1(BaseClassifier):

    def __init__(self, num_classes, channels, fs):
        super(EEGMultiScaleClassifier_V1, self).__init__()
...
# Ora faccio la V2 facendo veramente un calcolo multi-scala
# Quindi mettendo più kernel temporali di lunghezza diversa nello stesso livello
# (per farlo devo fare un blocco di tipo V2)
class EEGMultiScaleClassifier_V2(BaseClassifier):

    def __init__(self, num_classes, channels, fs):
        super(EEGMultiScaleClassifier_V2, self).__init__()
        ...
        self.block_1 = EEGConvBlock_V2(channels, F1, F2, kernel_time)
        self.avg_pool_1 = nn.AvgPool2d(kernel_size=(1,4))
        self.dropout_1 = nn.Dropout(0.5)
        self.block_2 = EEGConvBlock_V2(F2, F2, F2, kernel_time//4)
        self.avg_pool_2 = nn.AvgPool2d(kernel_size=(1,8))
        self.dropout_2 = nn.Dropout(0.5)  
        self.conv_final = nn.Conv2d(1, 1, (1, 8), bias=False)  
        self.batch_norm = nn.BatchNorm2d(1)   
